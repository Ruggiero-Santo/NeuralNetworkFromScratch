function makePlotCup(history, varargin)
    ...
    axis([1 size(train_acc, 2) 0 max(train_acc)]);
    xlabel('Epoch');
    ylabel('MEE');
    if size(history,1) == 6
        ...
        txt1 = strcat('Min.Acc-Tr = ',mat2str(min(train_acc),3),...
                    ' Min.Acc-Va = ',mat2str(min(valid_acc),3),...
                    ' Min.Acc-Te = ',mat2str(min(valid_acc),3));
    else
        if size(history,1) == 4
            plot(valid_acc(2:end));
